...
import { person, skills, technologies, quote } from "../data";
      ...
      <div className="mx-auto max-w-[1240px]">
        <div ref={ref} className="reveal">
          ...
          <h2 className="font-display text-[32px] font-bold leading-[1.12] text-white sm:text-[46px]">
            Rooted in the field,
            <br />
            building for the <em className="italic text-gold">future</em>
          </h2>
        </div>
        ...
        <div className="mt-14 grid auto-rows-[minmax(140px,auto)] grid-cols-1 gap-5 sm:grid-cols-4">
          ...
          <div className="col-span-1 rounded-2xl border border-white/[0.07] bg-dark-3 p-8 sm:col-span-2 sm:row-span-2 sm:p-10">
            <div className="mb-4 text-[10px] uppercase tracking-[2px] text-gold">Background</div>
            {person.bio.map((p, i) => (
          ...
          <div className="relative col-span-1 h-[260px] overflow-hidden rounded-2xl border border-white/[0.07] bg-dark-3 sm:col-span-2 sm:h-auto">
            ...
            <div className="pointer-events-none absolute bottom-0 left-0 right-0 bg-gradient-to-t from-dark-3 via-dark-3/70 to-transparent p-5">
              <div className="text-[10px] uppercase tracking-[2px] text-gold">Based in</div>
          ...
          <div className="col-span-1 rounded-2xl border border-white/[0.07] bg-dark-3 p-7 sm:col-span-2">
            <div className="mb-4 text-[10px] uppercase tracking-[2px] text-gold">What I work on</div>
            <div className="flex flex-wrap gap-2.5">
              {skills.map((s) => (
                <span
                  key={s}
                  className="rounded-full border border-gold/30 bg-gold/[0.06] px-4 py-1.5 text-[12px] tracking-wide text-gold/85"
                >
                  {s}
                </span>
              ))}
          ...
          {/* tech icons */}
          <div className="col-span-1 rounded-2xl border border-white/[0.07] bg-dark-3 p-7 sm:col-span-2">
            <div className="mb-4 text-[10px] uppercase tracking-[2px] text-gold">Tech stack</div>
            <div className="flex flex-wrap gap-3">
              {technologies.map((t) => (
                <div
                  key={t.name}
                  title={t.name}
                  className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/[0.07] bg-white/90 p-2.5 transition-transform hover:-translate-y-1 hover:border-gold/30"
                >
          ...
          <div className="col-span-1 flex flex-col justify-center rounded-2xl border border-white/[0.07] bg-dark-3 p-8 sm:col-span-4">
            <span className="mb-4 block font-display text-5xl leading-none text-gold/25">"</span>
            <p className="max-w-2xl font-display text-[18px] italic leading-[1.55] text-white/85 sm:text-[20px]">
              {quote.text}
            </p>
            <div className="mt-4 text-[11px] uppercase tracking-[3px] text-gold">{quote.author}</div>
          </div>
        </div>
      </div>
    </section>
